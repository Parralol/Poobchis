...
public class Player{
    ...
    /**
     * Costructor de la clase Player
     * @param name
     * @param color
     */
    public Player(String name, String color, ArrayList<Ficha> fichasDelJugador){
        this.name = name;
        this.color = color;
        poderesFichas = new Comodin();
        jail = new Carcel(color);
        this.fichaJugador = fichasDelJugador;
    ...
    /**
     * Segundo contructor (perimite iniciar con fichas al azar)
     * @param name
     * @param color
     */
    ...
    public void killCard(Ficha card){
        ...
        // Meterle una excepcion en caso de dado que se intente una ficha que no corresponda al jugador
    }

    /**
     * Elige las fichas al azar
     * @return
     */
    public ArrayList<Ficha> randomFichas(){
    ...
    /**
     * lee el comodin para que se active le poder en las fichas
     * @param x
     */
    ...
    public void updatePosition(Ficha ficha, int positionFicha){
        for(int i = 0; i < fichaJugador.size(); i++){
            if (fichaJugador.get(i) == ficha){
                System.out.println(positionFicha);
                fichaJugador.get(i).setPositionFicha(positionFicha);
                System.out.println(fichaJugador.get(i).getPositionFicha());
            }
        }
    }
} 
